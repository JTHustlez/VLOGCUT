...
export function TimelinePlayhead({
  ...
  return (
    ...
    >
      {/* The playhead line spanning full height - NEON CYAN with glow */}
      <div
        className="absolute left-0 w-0.5 cursor-col-resize h-full bg-primary"
        style={{
          boxShadow: isSnappingToPlayhead 
            ? '0 0 8px hsl(185, 100%, 50%, 0.8), 0 0 16px hsl(185, 100%, 50%, 0.4)'
            : '0 0 6px hsl(185, 100%, 50%, 0.6), 0 0 12px hsl(185, 100%, 50%, 0.3)',
        }}
      />

      {/* Playhead dot indicator at the top (in ruler area) - NEON style */}
      <div
        className="absolute top-1 left-1/2 transform -translate-x-1/2 w-3 h-3 rounded-full bg-primary border-2 border-primary/50"
        style={{
          boxShadow: '0 0 8px hsl(185, 100%, 50%, 0.7)',
        }}
      />
    </div>
  );
