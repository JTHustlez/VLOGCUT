      ...
      <div className="pt-6 px-6 flex items-center justify-between w-full h-16">
        ...
        >
          ...
          <Image
            ...
            width={20}
            height={20}
            className="w-5 h-5 transition-all group-hover:drop-shadow-[0_0_6px_hsl(45,67%,52%,0.5)]"
          />
      ...
      <main className="max-w-6xl mx-auto px-6 pt-6 pb-6">
        ...
        {isSelectionMode && sortedProjects.length > 0 && (
          <button
            ...
            className="w-full hover:cursor-pointer gap-2 mb-6 p-4 bg-secondary/30 rounded-lg border border-border/50 items-center flex hover:border-primary/30 transition-colors"
            tabIndex={0}
          >
...
function ProjectCard({
  ...
  const cardContent = (
    <Card
      className={`overflow-hidden bg-card/50 border-border/30 p-0 transition-all duration-200 ${
        isSelectionMode && isSelected 
          ? "ring-2 ring-primary shadow-[0_0_15px_hsl(185,100%,50%,0.2)]" 
          : "hover:border-primary/30 hover:shadow-[0_0_20px_hsl(185,100%,50%,0.1)]"
      }`}
    >
      <div
        className={`relative aspect-square bg-secondary/50 transition-opacity ${
          isDropdownOpen ? "opacity-65" : "opacity-100 group-hover:opacity-90"
        }`}
      >
        {isSelectionMode && (
          <div className="absolute top-3 left-3 z-10">
            <div className="w-5 h-5 rounded-full bg-background/80 backdrop-blur-xs border border-primary/30 flex items-center justify-center">
              <Checkbox
                checked={isSelected}
        ...
        <div className="absolute inset-0">
          {isLoadingThumbnail ? (
            ...
            <div className="w-full h-full bg-secondary/50 flex items-center justify-center">
              <Video className="h-12 w-12 shrink-0 text-muted-foreground/50" />
            </div>
          )}
        </div>
      </div>
            ...
            >
              <DropdownMenuTrigger asChild>
                <Button
                  variant="text"
                  size="sm"
                  className={`size-6 p-0 transition-all shrink-0 ml-2 hover:text-primary ${
                    isDropdownOpen
                      ? "opacity-100"
...
function CreateButton({ onClick }: { onClick?: () => void }) {
  return (
    <Button variant="primary" className="flex" onClick={onClick}>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-muted-foreground mb-8 max-w-md">
        Start creating your first project. Import media, edit, and export
        professional videos with ease.
      </p>
      <Button variant="primary-gradient" size="lg" className="gap-2" onClick={onCreateProject}>
